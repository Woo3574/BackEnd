# 지도 학습 : 입력 데이터와 그에 해당 하는 정답을 사용해 모델을 학습하는 방법
# 학습 데이터에서 입력과 정답의 관계를 학습하여 새로운 입력에 대해 정확히 예측할 수 있도록 하는 것
# 훈련 세트 : 모델 학습용 데이터
# 테스트 세트 : 모델 평가용 데이터
# 즉, 같은 데이터로 학습과 평과를 진행하면 모델 데이터를 학습 해버렸기 때문에 정확한 평가 불가
...
# 생선의 길이와 무게를 fish_data (2차원 리스트)로 만들고, 도미와 빙어 레이블으  fish_target에 저장
...
index = np.arange(49) # 0 ~ 48 까지의 인덱스 생성
...
# 훈련 세트와 테스트 세트 나누기
train_input = fish_arr[index[:35]] # 0 ~ 34번째 까지를 훈련 세트로 지정
...
if train_score > test_score:
    ...
    print("모델이 훈련 세트를 충분히 학습하지 못했습니다.")
else:
